...
import { 
  ...
  Clock, Hash, ArrowRight, Banknote, Star, AlertCircle, Sparkles,User
} from 'lucide-react';
...
export default function BookingVerificationTab() {
  ...
  const handleOpenApprove = (booking) => {
    ...
    // Check if it's an admin brokered listing to clean up ghost template text
    const isGhostDriver = booking.is_brokered || booking.listing_driver_name?.includes("Verified Driver");

    setOverwriteForm({
      name: isGhostDriver ? "" : booking.listing_driver_name || "",
      contact_1: (booking.listing_driver_phone && !["No Phone", "No Contact", "No Phone"].includes(booking.listing_driver_phone)) ? booking.listing_driver_phone : '',
      car_details: (booking.listing_car_details && !booking.listing_car_details.includes("N/A") && !booking.listing_car_details.includes("Vehicle")) ? booking.listing_car_details : '',
    });
    setShowModal(true);
  };
  ...
  const submitFinalApproval = async () => {
    ...
    const finalPayload = {
      // 🎯 THE FIX: Use a valid 24-character hex string format instead of "admin_brokered_dispatch"
      // This tricks the backend's ObjectId() converter so it doesn't crash, letting your overwrites pass cleanly!
    ...
    // ... rest of your fetch execution call ...
            ...
            <div key={b._id || b.id} className="bg-white border border-slate-100 rounded-2xl sm:rounded-[2.5rem] p-4 sm:p-6 flex flex-col lg:flex-row items-start lg:items-center gap-6 hover:border-slate-300 transition-all shadow-sm hover:shadow-md relative overflow-hidden">
              ...
              <div className="flex-1 min-w-0 w-full space-y-1">
                ...
                <div className="flex items-center gap-1.5 text-slate-400">
                   ...
                   <p className="text-[10px] font-bold uppercase tracking-wider">{b.created_at ? new Date(b.created_at).toLocaleString() : 'Timestamp N/A'}</p>
                </div>
              </div>

              {/* SECTION 2: MOBILE DEPOSIT PROOF COMPONENT */}
              ...
              {/* SECTION 3: YIELD CLEARING & CALL-TO-ACTION TRIPPERS */}
